...
class MainWin:
    def __init__(self, root):
        ...
        self.root.configure(bg="#2E3B4E")  # Dark blue background for tech feel
        
        img = Image.open(r"Images\logo4_GPT-removebg.png")
        img = img.resize((90, 90), Image.LANCZOS)
        self.photoimg = ImageTk.PhotoImage(img)

        title_lbl = Label(self.root, text="HealthCare ChatBot", font=("times new roman", 30, "bold"), bg="#1A2A38", fg="white")
        title_lbl.place(x=100, y=20, width=677, height=80)
        
        image_label = tk.Label(self.root, image=self.photoimg, bg="#2E3B4E")  # Dark blue background
        image_label.place(x=2, y=15)

        b1 = Button(self.root, text="About", cursor="hand2", command=self.about, font=("times new roman", 20, "bold"), bg="#4A6572", fg="white")
        b1.place(x=20, y=115, width=151, height=33)
        
        b2 = Button(self.root, text="Get Diagnosis", cursor="hand2", command=self.askBotFunc, font=("times new roman", 20, "bold"), bg="#4A6572", fg="white")
        b2.place(x=183, y=115, width=203, height=35)
        
        b3 = Button(self.root, text="Ask Advisor", cursor="hand2", command=self.askGpt, font=("times new roman", 20, "bold"), bg="#4A6572", fg="white")
        b3.place(x=400, y=115, width=211, height=35)
        
        b4 = Button(self.root, text="Exit", cursor="hand2", command=self.isExit, font=("times new roman", 20, "bold"), bg="#D96C6C", fg="white")
